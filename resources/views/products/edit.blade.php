...
<form enctype="multipart/form-data" class="px-40 mb-40" action="{{ route('products.update' ,$product->id)}}" method="POST">
    @method('put')
    @csrf
    <div class="space-y-12 ">
      <div class="border-b border-gray-900/10 pb-12">
        <h2 class="text-base font-semibold leading-7 text-gray-900">EDIT OBAT</h2>
        <p class="mt-1 text-sm leading-6 text-gray-600">Periksa Dengan Teliti</p>
        ...
        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
            ...
            <div class="mt-2">
                <input type="text" value="{{ old('harga' ,$product->harga) }}" name="harga" id="harga" placeholder="  25.000" class="@error('harga') is-invalid @enderror block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
            @error('harga')
                <p class="invalid-feedback">{{$message}}</p>
            @enderror
            </div>
            ...
            <div class="mt-2">
                <input id="deskripsi" value="{{ old('deskripsi' ,$product->deskripsi) }}" name="deskripsi" type="deskripsi" class="@error('deskripsi') is-invalid @enderror block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 h-60">
            @error('deskripsi')
                <p class="invalid-feedback">{{$message}}</p>
            @enderror
            </div>
            ...
            <div>    
                @if ($product->gambar != "")
                        <img class="w-50 my-3" src="{{ asset('uploads/products/'.$product->gambar) }}" alt="">
                    @endif
                    </div>
              </div>
        </div>
